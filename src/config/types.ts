...
export const teamModeConfigSchema = z.object({
  ...
  maxMemberTurns: z.number().int().min(1).default(500),
  ...
  mailboxPollIntervalMs: z.number().int().min(500).default(3000),
})
...
export const aionConfigSchema = z.object({
  ...
  personality: z.object({
    ...
    heartbeatMinMs: z.number().int().min(0).default(90_000).describe("Minimum gap between heartbeats (ms)."),
    heartbeatMaxMs: z.number().int().min(0).default(240_000).describe("Maximum gap between heartbeats (ms); heartbeats are jittered within [min, max]."),
    maxHeartbeatsPerSession: z.number().int().min(0).default(8).describe("Cap on heartbeat toasts per session to avoid spam."),
  }).default({
    enabled: true,
    entrance: true,
    transitions: true,
