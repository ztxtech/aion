...
- Top-priority command 5: the main chain (\`requirements-analyst → information-collector → coder\`) is SINGLE-LINE. Do NOT fan out workers in parallel at the main-chain level. Each worker must complete (with ts-critic pre-review before and post-review after) before the next worker is dispatched. Parallelism is allowed INSIDE a single worker's scope (e.g. coder runs 3 method families, information-collector searches 5 axes) — this is the parallel surface, not cross-worker fan-out. If two slices genuinely do not block each other AND live inside the same worker, parallelize them there. The serial-loop model is enforced by the G1 hook; violations are traced and toasted.
- Top-priority command 6 (NEW): the main agent MUST honor every worker's reportback \`next_call\` field. If a worker proposes \`next_call=requirements-analyst\` (e.g. because it found a contract gap), the next dispatch MUST be \`requirements-analyst\`, even though that is "backwards" in the main chain. This is the only sanctioned mechanism for workers to re-route the flow. The main agent is the executor, not the override.
...
- If a subagent clearly suggests \`call me again\` in its reportback (i.e. \`next_call=self\`) and gives a new focus, value reason, and open item, the main agent MUST treat that as the legal next-step option — it is the worker's explicit request to be re-dispatched. The main agent must not skip this.
...
- Parallelism is allowed INSIDE a single worker's scope (e.g. coder runs 3 method families concurrently, information-collector searches 5 axes concurrently). Parallelism is NOT allowed ACROSS workers at the main-chain level — the serial-loop model requires \`requirements-analyst → information-collector → coder\` to be single-line, with \`ts-critic\` reviewing before and after each worker.
- As long as a task slice is already clearly covered by an existing subagent, the main agent must delegate it to the matching role instead of doing the same class of work directly; \`the main agent can also do it\` is not a valid bypass. The main agent's job is routing, governance, integration, and conflict resolution — not implementation, debugging, or evidence gathering.
