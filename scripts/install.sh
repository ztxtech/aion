...
if [ -n "$LOCAL_TARBALL" ]; then
  ...
  API_RESP=$(curl -fsSL "https://api.github.com/repos/${GITHUB_OWNER}/${GITHUB_REPO}/releases/latest" 2>/dev/null || echo "")
  if [ -n "$API_RESP" ]; then
  ...
  if [ -z "$VERSION" ]; then
    fail "no GitHub Release found for ${GITHUB_OWNER}/${GITHUB_REPO}.

  Options:
    1. Create a release first:
       bash scripts/build.sh
       gh release create v0.1.0 release/aion-plugin-0.1.0.tar.gz --target dev

    2. Or install from a local tarball (testing):
